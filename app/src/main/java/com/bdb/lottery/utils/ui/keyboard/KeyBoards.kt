package com.bdb.lottery.utils.ui.keyboard
...
object KeyBoards {
    ...
    fun showSoftInput() {
        val imm =
            BdbApp.sApp.getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
                ?: return
    ...
    private fun getDecorViewInvisibleHeight(window: Window): Int {
        ...
        if (delta <= Sizes.getNavBarHeight() + Sizes.getStatusBarHeight()) {
    ...
    private fun getContentViewInvisibleHeight(window: Window): Int {
        ...
        return if (delta <= Sizes.getStatusBarHeight() + Sizes.getNavBarHeight()) {
            0
        } else delta
    }
