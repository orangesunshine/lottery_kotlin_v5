...
class LotDuplexAdapter constructor(
    ...
    override fun convert(holder: BaseViewHolder, item: LotDuplexData, payloads: List<Any>) {
        ...
        when (payloads[0]) {
            PAY_LOAD_BIG -> dxdsTag = 0//大
            PAY_LOAD_SMALL -> dxdsTag = 1//小
            PAY_LOAD_SINGLE -> dxdsTag = 2//单
            PAY_LOAD_DOUBLE -> dxdsTag = 3//双
            PAY_LOAD_NONE -> dxdsTag = -1//取消大小单双选中
        }
        if (-1 != preTag)
    ...
    private fun renderBall(
        ...
        rv.adapter?.let {
            ...
            if (it is LotDuplexSubAdapter) {
                it.notifyChangeWhenPlayChange(m11x5DanTuo && adapterPosition == 0, betTypeId, mSpanCount, item)
                ...
                override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
                    ...
                    parent.layoutManager?.let {
                        if (it is GridLayoutManager) {
                            ...
                            if (spanCount > 0 && (position / spanCount > 0 || (Lots.isK3HeZhi(betTypeId) && position > 3))) {
                                top = Sizes.dp2px(6f)
                            }
                        }
                    }
            ...
            rv.adapter = LotDuplexSubAdapter(
                ...
                if (leave?.leaveOut?.size ?: 0 > adapterPosition) leave?.leaveOut?.get(adapterPosition) else null
            ) {
                //号码球选中切换回调
                    ...
                    if (!selectedPositions.isNullOrEmpty()) {
                        if (adapterPosition == 0) {
                            ...
                            if (!it.isNullOrEmpty() && it.contains(selectedPositions[0])) {
                                mSubAdapters[1]?.notifySingleUnSelectedPositionWithPayLoads(selectedPositions[0])
                            }
                        } else if (adapterPosition == 1) {
                            //拖码
                            if (!selectedPositions.isNullOrEmpty() && it.contains(selectedPositions[0])) {
                                mSubAdapters[0]?.notifySingleUnSelectedPositionWithPayLoads(selectedPositions[0])
                            }
                        }
                    }
                }
    ...
    private fun renderDxds(
        ...
        if (dxdsVisible) {
            ...
            if (GAME.TYPE_GAME_PK8 == gameType || GAME.TYPE_GAME_PK10 == gameType) {
                holder.setBackgroundResource(R.id.lot_duplex_item_big, R.drawable.lot_duplex_sub_item_pk_dxds_selector)
                holder.setTextColorStateList(context, R.id.lot_duplex_item_big, R.color.lot_duplex_sub_item_dxds_text_selector)
                holder.setBackgroundResource(R.id.lot_duplex_item_small, R.drawable.lot_duplex_sub_item_pk_dxds_selector)
                holder.setTextColorStateList(context, R.id.lot_duplex_item_small, R.color.lot_duplex_sub_item_dxds_text_selector)
                holder.setBackgroundResource(R.id.lot_duplex_item_single, R.drawable.lot_duplex_sub_item_pk_dxds_selector)
                holder.setTextColorStateList(context, R.id.lot_duplex_item_single, R.color.lot_duplex_sub_item_dxds_text_selector)
                holder.setBackgroundResource(R.id.lot_duplex_item_double, R.drawable.lot_duplex_sub_item_pk_dxds_selector)
                holder.setTextColorStateList(context, R.id.lot_duplex_item_double, R.color.lot_duplex_sub_item_dxds_text_selector)
            }
        }
    }
    ...
    private fun renderLabel(
        ...
        if (!gone) {
            ...
            //选中背景
            holder.setBackgroundResource(R.id.lot_duplex_item_label_tv,
                when (gameType) {
            ...
            if (GAME.TYPE_GAME_11X5 == gameType) {
                ...
                if (GAME.TYPE_GAME_11X5 == gameType) holder.getView<TextView>(R.id.lot_duplex_item_label_tv).setEms(1)
                holder.setUnitedWH(R.id.lot_duplex_item_label_tv, 24f, label?.let { if (it.length > 3) 58f else if (it.length > 2) 48f else 38f } ?: 38f)
            } else {
                //其他横排：根据文字个数调整label宽度
                holder.setWidthWithUnit(R.id.lot_duplex_item_label_tv, label?.let { if (it.length > 3) 68f else if (it.length > 2) 48f else 38f } ?: 38f)
            }
        }
    ...
    private fun dxdsSelectedChange(holder: BaseViewHolder, tag: Int, selected: Boolean) {
        when (tag) {
            0 -> holder.setItemChildSelected(R.id.lot_duplex_item_big, selected)
            1 -> holder.setItemChildSelected(R.id.lot_duplex_item_small, selected)
            2 -> holder.setItemChildSelected(R.id.lot_duplex_item_single, selected)
            3 -> holder.setItemChildSelected(R.id.lot_duplex_item_double, selected)
        }
    }
    //endregion
    ...
    init {
        addChildClickViewIds(R.id.lot_duplex_item_big,
    ...
    fun notifyChangeWhenPlayChange(lotDuplexLd: LotDuplexLd) {
        ...
        m11x5DanTuo = betTypeId == 586 || betTypeId == 587 || betTypeId == 179 || betTypeId == 182 || betTypeId == 176//11选5组选胆拖的胆码
    ...
    fun renderHot(hot: HotData?) {
        ...
        if (mSubAdapters.isNotEmpty()) {
            for ((index, adapter) in mSubAdapters.valueIterator().withIndex()) {
                adapter.renderHot(if (hot?.coldHot?.size ?: 0 > index) hot?.coldHot?.get(index) else null)
            }
        }
    }
    //endregion
    ...
    fun renderLeave(leave: LeaveData?) {
        ...
        if (mSubAdapters.isNotEmpty()) {
            for ((index, adapter) in mSubAdapters.valueIterator().withIndex()) {
                adapter.renderLeave(if (leave?.leaveOut?.size ?: 0 > index) leave?.leaveOut?.get(index) else null)
            }
        }
    }
    //endregion
